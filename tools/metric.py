...
def mmcv_imread(filePath):
    img_bytes = mmengine.fileio.get(filePath)
...
def dir_metric(pred_path, gt_path, pred_suffix, gt_suffix, n_class):
    ...
    for item in files:
        name = item.split('.')[0]
        cur_pred = mmcv_imread(os.path.join(pred_path, name+pred_suffix))
        cur_gt = mmcv_imread(os.path.join(gt_path, name + gt_suffix))
