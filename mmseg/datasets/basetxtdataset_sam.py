...
def generate_points(img_path, spacing=20):
    binary_image = cv2.imread(img_path, 0)
    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    inside_points = []  # list1: 连通域内的点
    outside_points = []  # list2: 不在连通域的点
    ...
    for contour in contours:
        ...
        for dx in range(0, w // 2 + spacing, spacing):
            for dy in range(0, h // 2 + spacing, spacing):
                points = [
                    (center_x + dx, center_y + dy), (center_x - dx, center_y + dy),
                    (center_x + dx, center_y - dy), (center_x - dx, center_y - dy)
                ]
                for px, py in points:
                    if x <= px < x + w and y <= py < y + h and binary_image[py, px] != 0:
    ...
    for i in range(0, width, spacing):
        for j in range(0, height, spacing):
            if not binary_image[j, i]:
                outside_points.append([i, j])

    return np.array(inside_points), np.array(outside_points)


@DATASETS.register_module()
class TXTSAMDataset(BaseDataset):
    ...
    # 备注1定义数据集函数这里需要先设定这个颜色和类别的信息，根据类别来制定
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if not osp.isdir(self.ann_file) and self.ann_file:
            ...
            for line in lines:
                ...
                pcam = plab.replace('label', 'cam_vis')
                inside_points, outside_points = generate_points(pcam)
                input_point = np.asarray(list(inside_points)+list(outside_points))
                input_label = np.array([1 for i in range(len(inside_points))]+[0 for i in range(len(outside_points))])
                ...
                data_info['json'] = jsondata['/'.join(pimg.split('/')[-4:]).replace('.png', '.tif').replace('whub_seg_1024', 'whub_seg')]
                data_info['point_coords'] = input_point
                data_info['point_labels'] = input_label
